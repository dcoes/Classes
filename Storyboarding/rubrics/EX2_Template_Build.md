...
## The 1% tolerance, and why it is not negotiable

A panel is inside tolerance if `width ÷ height` lands within 1% of the target.
...
**The row check is worth 8 points on purpose.** It is the only part of this assignment that is a *decision* rather than a measurement, and it is where the actual lesson lives: a taller frame fits fewer rows on a page, because frame shape has consequences that run all the way out to logistics. A student who builds three perfect grids and never notices the 1.33 overflow has done the arithmetic without understanding what it was telling them.

Either resolution earns full credit. **Not noticing does not.**
...
| No room left for camera notation | Up to 4 of the field points |

---

## Milestone note

EX2 is single-stage and carries **no milestone cap**.

It is, however, a **prerequisite** for everything downstream. EX3, EX4, EX5, PR1 and PR4 are all delivered on these templates. Boards submitted on a template that was never graded are boards on an unverified ratio, and they will be assessed as such.
