# PR1 City Park Sequence: Rubric
**100 points · Band PR · Assessed against CO1, CO2, CO3, CO4, CO5**
**Brief:** [`assignments/PR1_City_Park_Sequence.md`](../assignments/PR1_City_Park_Sequence.md)

> Simple by design. This assignment assesses **continuity and staging under constraint**: one location, few characters, nothing exotic. What's being tested is whether you can keep a space coherent across a cut, not whether you can invent a world.

---

## Milestone chain

| Stage | Pts | Due |
|---|--:|---|
| **1 · Beat sheet** | 15 | Session 5 |
| **2 · Thumbnails** | 20 | Session 6 |
| **3 · Rough boards** | 25 | Session 7 |
| **4 · Final boards** | 40 | Session 8 |

> **Each milestone gates the next. A stage submitted without the preceding stage is capped at 50% of that stage's available points.**
>
> The cap cascades. Skipping thumbnails costs you the 20 points *and* caps your roughs at 12.5: 32.5 points to save one afternoon. The arithmetic is deliberate.

---

## 1 · Beat sheet: 15 pts · CO3

| | Pts | Full credit |
|---|--:|---|
| Events tabulated | 8 | Every beat in the sequence listed in order, one line each. Nothing merged, nothing skipped. |
| Beat → panel mapping | 7 | Each beat carries an intended panel count. The mapping is a decision, not a guess. |

*This stage is the direct assessment of CO3: tabulating storyboards based on certain events. It is not administrative overhead; it's the objective.*

## 2 · Thumbnails: 20 pts · CO1

| | Pts | Full credit |
|---|--:|---|
| Coverage | 7 | The whole sequence is thumbnailed, not the interesting parts of it. |
| Divergence | 7 | At least two beats staged two ways. **A variant counts only if it changes shot size, camera angle, blocking, or which character the frame is on.** Redrawing the same idea more neatly counts as one version, not two. |
| The rejected one | 4 | For each of those beats: both versions submitted, the choice marked, and **one line on what the rejected version did better.** |
| Format discipline | 2 | Delivered on the EX2 **working sheet**: small panels, many to a page. Panels at final-board size are out of spec. |

*The exploration is what's graded, not the drawings. Coverage, divergence and the marked choice are all countable, which is what makes it possible to grade brainstorming at all.*

*The rejected-option line is the cheapest and most useful requirement on this rubric. A student cannot write it without having genuinely considered both, and it moves the graded object from the drawing to the reasoning.*

## 3 · Rough boards: 25 pts · CO1, CO4

| | Pts | Full credit |
|---|--:|---|
| Action blocked | 8 | Character positions and movement are legible in every panel. |
| Continuity established | 10 | The 180° line is chosen and held; screen direction is consistent; eyelines match. |
| Silhouette readability | 7 | Poses read filled solid black. A stranger can name the action. |

## 4 · Final boards: 40 pts · CO1, CO2, CO4

| | Pts | Full credit |
|---|--:|---|
| **Continuity holds across the sequence** | 12 | Line, screen direction, eyeline matches, match-on-action. Any crossing of the line is deliberate and earned. |
| **Composition** | 10 | Figure/ground separation; one dominant element per panel; correct headroom/lookroom/leadroom; consecutive panels contrast. |
| **Notation and terminology** | 8 | Shot sizes and angles named correctly. Camera arrows distinguished from motion arrows. Panels numbered by the H1 convention. *(CO2)* |
| **Silhouette readability** | 6 | The standing standard, applied to the finished set. |
| **Document completeness** | 4 | Correct ratio, panel and shot numbers, action described beneath each panel. Delivered on the EX2 **presentation sheet**. |

> **A final panel that is a scan, trace, or upscale of a thumbnail is not a final panel. It scores as a missing panel.** The thumbnail's job is to be thrown away. If you are protecting one, it has stopped doing its job.

## Critique participation: folded into the above · CO5

Peer notes given during the City Park critique session count as CO5 evidence and are recorded. A student who submits work and gives no notes has met CO1–CO4 and not CO5.

---

**Total: 100**

---

## What is not graded

**Rendering.** Not once, anywhere on this rubric. A crude panel that stages the beat clearly outscores a polished panel that doesn't.

**Ambition.** This is a constrained brief on purpose. Adding a car chase to City Park does not earn points and usually costs them, because continuity is harder to hold and the assignment is *about* holding continuity.

---

## The one thing that fails most often

**Figure/ground separation.** Mid-value figure on mid-value ground. It is the number one reason student panels fail and it has nothing to do with drawing ability. Run the squint test (H3) before you submit.
