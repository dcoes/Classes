# PR4 Final: Script to Animatic
**30% of the course grade · Assigned Class 9 · Due Class 14 · The largest assignment in the course**
**Rubric:** [`rubrics/PR4_Final_Script_to_Animatic.md`](../rubrics/PR4_Final_Script_to_Animatic.md)
**Handouts:** H1 · H2 · H3 · H4 · H5 · H6: all six

---

## Start here

> **What experience do you want the audience to have?**
>
> **What emotion?**

Write the answer down before you do anything else. One sentence each.

Everything below is execution. This is the only part that's intent, and every decision you make from here should be traceable back to it. When you're stuck, and you will be, come back to this sentence and ask which option serves it.

---

## The assignment

A script becomes a beat sheet, becomes a shot list and floor plans, becomes thumbnails, becomes rough boards, becomes final boards, becomes a timed animatic with sound.

Everything this course teaches, end to end, on one sequence.

---

## Your script

**Default path:** you receive an assigned script. `[ASSIGNED SCRIPT: TBD]`

**Nobody is stranded.** If you do nothing, you have a script and you can start today.

**Pitch path:** you may pitch your own premise to replace the assigned script. See the gate below.

---

## The pitch gate

In **Class 13**, before your final is finished, you pitch. This is a separate assignment with its own rubric: see [`The Pitch`](../rubrics/PR5_The_Pitch.md).

If you want to replace the assigned script with your own premise, this is where you ask.

**The gate is real. A pitch can be declined.**

A declined pitch costs you nothing but the swap. You proceed on the assigned script with no penalty of any kind. A gate that always opens teaches nothing, so this one doesn't.

Two things worth knowing:

1. **Passing the gate is not worth points.** The *quality of the pitch* is. A student who pitches brilliantly and gets declined scores higher than one who pitches badly and gets approved.
2. **The pitch is also a milestone.** It happens before the final is done specifically so problems surface while there is still time to fix them. Use it that way.

---

## Milestone chain

| Stage | % of this assignment | Due |
|---|--:|---|
| **1 · Script markup + beat sheet** | 13% | Class 10 |
| **2 · Shot list + floor plans** | 10% | Class 11 |
| **3 · Thumbnails** | 12% | Class 12 |
| **4 · Rough boards** | 15% | Class 13 |
| **5 · Final boards** | 22% | Class 14 |
| **6 · Animatic** | 23% | Class 14 |
| **7 · Critique participation** | 5% | Class 14 |

> **Each milestone gates the next. A stage submitted without the preceding stage is capped at half its available percentage.**

**Read this arithmetic before you decide to skip anything.** Across seven stages the cascade is severe on purpose. A student who skips thumbnails and roughs to arrive with finished boards loses 27% outright and has their 22% of final boards capped at 11%, which is a **38-point hit on a 100-point rubric.**

Skipping process here is not a shortcut. It is a failing grade with extra steps.

---

## What each stage is

**1 · Script markup + beat sheet.**

*The markup.* Go through the script and **highlight it in colour, one colour per shot size.** Wides one colour, mediums another, close-ups another. Then annotate in the margin: angle, framing, and any camera move you intend.

You are reading the script as a director reads it: deciding, line by line, how close the camera is when that line happens. Do this before you draw anything, because it is much faster to change a highlighter mark than a panel, and because a script marked this way makes the shot list almost write itself.

Two things fall out of it immediately, and both are the point: **long unbroken runs of one colour** mean you have not varied your coverage, and **a colour change on every line** means you are cutting for the sake of cutting.

*The beat sheet.* Every event in order, one line each, with an intended panel count. Plus: name the structure. Where does it turn? What's the hook? What is this sequence *about*?

**2 · Shot list + floor plans.** Framing, angle, and script reference for every intended shot, using correct terminology (H2). Then an **overhead diagram for every location**, with camera positions numbered and the line of action marked.

> The floor plan is the most underused tool in this discipline. Nobody makes you do it. Everybody who does it has fewer problems. Here, someone is making you do it.

**3 · Thumbnails.** The whole sequence, not the fun parts, on your **EX2 working sheet.**

**The first pass happens in class, in Class 12, timed: roughly ninety seconds a beat, on paper, photographed and posted before you leave.** That sprint sheet is that class's exit ticket and it is part of this milestone. You cannot render anything in ninety seconds, which is exactly why it's ninety seconds.

Then, for **five designated beats**, stage each one **three ways.** A variant only counts if it changes **shot size · camera angle · blocking · which character the frame is on.** Redrawing the same idea more neatly is not a variant.

Mark your choice for each, and write **one line on what the rejected version did better.**

> **A final panel that is a scan, trace, or upscale of a thumbnail is not a final panel. It scores as a missing panel.**
>
> The thumbnail's job is to be thrown away. If you are protecting one, it has stopped doing its job, and a 0.9″ sketch blown up to panel size looks exactly like what it is.

**4 · Rough boards.** Action and dialogue blocked. Continuity established: line held, screen direction consistent, eyelines matched. Poses read in silhouette (H4, H6).

**5 · Final boards.** On your EX2 presentation sheet, notated per H1, with H3 and H6 actually run. Every character wants something in every panel. Panels numbered to the script's scene numbers.

**6 · Animatic.** Timed, with sound. Full spec in H5 and in the Animatic brief.

**7 · Critique participation.** Substantive notes to at least three peers across the final critiques. Plus documentation of the notes *you* received and one line on what you changed.

---

## The standards, restated

You have heard all of these before. They are what this assignment is for.

- **Clarity over beauty.** The question is never "is this well drawn." It's "does this communicate, and did you decide it on purpose."
- **Silhouette readability.** Fill it black. Can a stranger name the action?
- **Composition is about two frames, not one.** Every panel is a question about how it relates to the one before it.
- **Every character wants something in every panel.** If you can't say what, the panel is decoration.
- **Timing is a decision.** Use the stopwatch.

---

## What is not graded

**Rendering, at any stage.**

**Length beyond the brief.** A longer sequence is not a better one, and it's much harder to hold continuity across. Restraint reads as judgment here, not as laziness.

**Nerves at the pitch.** Defending your work instead of hearing the note does.

---

## One honest note about the field

The part of this job that is *"make a picture of a thing"* is under real pressure. I'm not going to tell you otherwise.

The part that is *"decide what the picture should be, and know why"* is the part this assignment is built around, and it's the part that's still yours. That isn't reassurance. It's the reason this course grades clarity and judgment instead of rendering, and it's the reason the pitch is on the syllabus at all.
